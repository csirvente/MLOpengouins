...
def sample_penguins_csv():
    ...
    data = {
        ...
        * 3,
        ...
        "flipper_length_mm": [181.0, 210.0, 195.0, 186.0, 200.0, 197.0, 183.0, 212.0, 196.0, 185.0]
        * 3,
        "body_mass_g": [
            3750.0,
            4500.0,
...
class TestFullPipeline:
    ...
    def test_pipeline_reproducibility(self, sample_penguins_csv):
        ...
        for _ in range(2):
            ...
            model = train_model(
                X_train_transformed, y_train, model=KNeighborsClassifier(n_neighbors=3)
            )
            score = evaluate_model(model, X_test_transformed, y_test)
            scores.append(score)
    ...
    def test_pipeline_handles_validation_set(self, sample_penguins_csv):
        ...
        X_val_transformed = preprocess_data(
            X_val, fit=False, preprocessing_model=preprocessing_model
        )
